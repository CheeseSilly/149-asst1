...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  //
  __cs149_vec_float x;
  __cs149_vec_int y, count, allOne, allZero;
  __cs149_vec_float res, v999 = _cs149_vset_float(9.999999f);
  __cs149_vec_float zero = _cs149_vset_float(0.0f);
  __cs149_mask maskAll, maskIsZero, maskIsNotZero, maskCountIsPositive, mask999,
      maskCountIsNotPositive;
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    // All ones
    if (i + VECTOR_WIDTH > N) {
    ...
    // Set mask according to predicate
    _cs149_veq_float(maskIsZero, x, zero, maskAll); // if (y == 0) {

    // output[i] = 1.f;
    _cs149_vset_float(res, 1.f, maskIsZero);
    ...
    // float result = x;
    // int count = y - 1;
    _cs149_vmove_float(res, x, maskIsNotZero);
    allOne = _cs149_vset_int(1);
    _cs149_vsub_int(count, y, allOne, maskIsNotZero);
    allZero = _cs149_vset_int(0);
    // while(count>0)
    // result *= x;
    // count--;
    while (1) {
      _cs149_vgt_int(maskCountIsPositive, count, allZero, maskIsNotZero);
      int cnt = _cs149_cntbits(maskCountIsPositive);

      if (cnt == 0) {
        break;
      }
      _cs149_vmult_float(res, res, x, maskCountIsPositive);
      _cs149_vsub_int(count, count, allOne, maskCountIsPositive);
      // cnt = _cs149_cntbits(maskCountIsPositive);
    }
    // if (result > 9.999999f) {
    //   result = 9.999999f;
    // }
    _cs149_vgt_float(mask999, res, v999, maskIsNotZero);
    _cs149_vmove_float(res, v999, mask999);
...
float arraySumVector(float *values, int N) {
  __cs149_vec_float vec, partialSum;
  __cs149_mask maskAll = _cs149_init_ones();
  partialSum = _cs149_vset_float(0.0f);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    _cs149_vload_float(vec, values + i, maskAll);

    _cs149_vadd_float(partialSum, partialSum, vec, maskAll);
  }

  float result[VECTOR_WIDTH];
  _cs149_vstore_float(result, partialSum, maskAll);
